...
func _spawn_circle(ind: Dictionary) -> void:
	...
	(ind["circles"] as Array[Dictionary]).append({
		"pos":           edge_pos,
		"direction":     dir,
		"radius":        radius,
...
func _random_edge_point() -> Vector2:
	match randi() % 4:
		0: return Vector2(randf_range(0.0, _screen_size.x), 0.0)
		1: return Vector2(randf_range(0.0, _screen_size.x), _screen_size.y)
		2: return Vector2(0.0, randf_range(0.0, _screen_size.y))
		_: return Vector2(_screen_size.x, randf_range(0.0, _screen_size.y))
	...
	for ind: Dictionary in _indicators:
		...
		for circle: Dictionary in circles:
			...
			if not (circle["exploding"] as bool):
				...
				col.a = alpha * 0.85
				draw_arc(ind_pos, (circle["radius"] as float) * (1.0 + t * 3.0), 0.0, TAU, 36, col, 2.0)

				col.a = alpha * 0.40
				draw_arc(ind_pos, (circle["radius"] as float) * (1.0 + t * 5.5), 0.0, TAU, 36, col, 1.0)
